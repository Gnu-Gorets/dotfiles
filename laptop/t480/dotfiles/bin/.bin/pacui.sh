...
# shellcheck disable=SC1117,SC2086                                                  # exclude MANY false-positive shellcheck warnings (see above)
...
function func_u
{
        ...
        then
            ...
            if ( wget --spider $(echo "$server") &>/dev/null )                      # the "wget --spider" command gets executed in any case in order to check its output.
            then
...
function func_m
{
        ...
        if ( sudo find /tmp/pacui* &>/dev/null )                                    # check if "pacui*" file or directory is found in /tmp/ directory
        then
            echo " deleting PacUI cache ..."
            sudo rm --recursive --dir /tmp/pacui*                                   # here the "rm -r" command is needed, because both directories and files have to be removed. "unlink" and "rmdir" do not work here.
            echo ""
        fi
                ...
                fzf -i \
                    ...
                    --preview '
                        ...
                        then
                            ...
                            elif ( test -n '$AUR_Helper' )                          # preview window of fzf requires checking with "test": check whether internet connection is up.
                            then
                    ...
                    "$(
                        ...
                        then
                            ...
                            echo "--preview-window=bottom:55%:wrap"
                        fi
                    )" \
...
function func_r
{
        ...
        pkg="$( sort -k1,1 -u /tmp/pacui-packages-group | fzf -i --multi --exact --no-sort --select-1 --query="$argument_input" --cycle --reverse --bind=right:half-page-down,left:half-page-up --margin="4%,1%,1%,2%" --info=inline --no-unicode --preview '
            ...
            ' "$( if (( $(tput cols) >= 125 )); then echo "--preview-window=right:55%:wrap"; else echo "--preview-window=bottom:55%:wrap"; fi )" --header="TAB key to (un)select. ENTER to remove. ESC to quit." --prompt='Enter string to filter list > ' | awk '{print $1}' )"
                ...
                case ${answer:-n} in                                                # if ENTER is pressed, the variable "answer" is empty. if "answer" is empty, its default value is "n".
                    ...
                    n|N|no|NO|No )                                                  # do this, if "answer" is n or N or no or NO or No
                        ...
                        do
                            ...
                            pkg="$( echo "$pkg_remove_backup" | fzf -i --multi --exact --no-sort --query="$argument_input" --reverse --bind=right:half-page-down,left:half-page-up --margin="4%,1%,1%,2%" --info=inline --no-unicode --preview '
                                if ( pacman -Qq {1} &>/dev/null )                   # check, if selected line is a locally installed package
                                then
                                    echo -e "\e[1mInstalled package info: \e[0m"
                                    pacman -Qi {1} --color always
...
function func_t
{
        ...
        pkg="$( sort -k1,1 -u /tmp/pacui-packages-local | fzf -i --exact --no-sort --select-1 --query="$argument_input" --cycle --reverse --bind=right:half-page-down,left:half-page-up --margin="4%,1%,1%,2%" --info=inline --no-unicode --preview '
            ...
            ' "$( if (( $(tput cols) >= 125 )); then echo "--preview-window=right:55%:wrap"; else echo "--preview-window=bottom:55%:wrap"; fi )"  --header="ENTER for dependency tree. ESC to quit." --prompt='Enter string to filter list > ' | awk '{print $1}' )"
        ...
        then
            ...
            then
                ...
                pactree --color --ascii "$pkg" | fzf -i --multi --exact --no-sort --ansi --reverse --bind=right:half-page-down,left:half-page-up --margin=4%,1%,1%,1% --info=inline --no-unicode --preview '
                    echo -e "\e[1mInstalled package info: \e[0m"
                    pacman -Qi "$( echo -e {} | sed "s/[\|\`\ -]*//" | cut -d " " -f 1 )" --color always
                    ' "$( if (( $(tput cols) >= 125 )); then echo "--preview-window=right:55%:wrap"; else echo "--preview-window=bottom:55%:wrap"; fi )" --header="Local Dependency Tree of \"$pkg\". ESC to quit." --prompt='Enter string to filter list > ' > /tmp/pacui-t

            else

                # if $pkg is not a locally installed package, "pactree --sync" shows reverse dependency tree of repository packages:
                pactree --color --ascii --sync "$pkg" | fzf -i --multi --exact --no-sort --ansi --reverse --bind=right:half-page-down,left:half-page-up --margin=4%,1%,1%,1% --info=inline --no-unicode --preview '
                    ...
                    ' "$( if (( $(tput cols) >= 125 )); then echo "--preview-window=right:55%:wrap"; else echo "--preview-window=bottom:55%:wrap"; fi )" --header="Repository Dependency Tree of \"$pkg\". ESC to quit." --prompt='Enter string to filter list > ' > /tmp/pacui-t
...
function func_rt
{
        ...
        pkg="$( sort -k1,1 -u /tmp/pacui-packages-local  | fzf -i --exact --no-sort --select-1 --query="$argument_input" --cycle --reverse --bind=right:half-page-down,left:half-page-up --margin="4%,1%,1%,2%" --info=inline --no-unicode --preview '
            ...
            ' "$( if (( $(tput cols) >= 125 )); then echo "--preview-window=right:55%:wrap"; else echo "--preview-window=bottom:55%:wrap"; fi )"  --header="ENTER for reverse dependency tree. ESC to quit." --prompt='Enter string to filter list > ' | awk '{print $1}' )"
        ...
        then
            ...
            then
                ...
                # explain the " echo {} | sed 's/^[|`- ]*//g' | cut -d ' ' -f 1 " command used below:
                # first echo selected line in fzf. then, remove all symbols from the beginning of the line, which does not belong to the package name. if there are multiple package names (e.g. with "provides") in 1 line all other (except for the first package name) are cut from the result.
                pactree --color --ascii --reverse "$pkg" | fzf -i --multi --exact --no-sort --ansi --reverse --bind=right:half-page-down,left:half-page-up --margin=4%,1%,1%,1% --info=inline --no-unicode --preview '
                    echo -e "\e[1mInstalled package info: \e[0m"
                    pacman -Qi "$( echo -e {} | sed "s/[\|\`\ -]*//" | cut -d " " -f 1 )" --color always
                    ' "$( if (( $(tput cols) >= 125 )); then echo "--preview-window=right:55%:wrap"; else echo "--preview-window=bottom:55%:wrap"; fi )" --header="Local Reverse Dependency Tree of \"$pkg\". ESC to quit." --prompt='Enter string to filter list > ' > /tmp/pacui-rt

            else

                # if $pkg is not a locally installed package, "pactree --sync" shows reverse dependency tree of repository packages:
                pactree --color --ascii --sync --reverse "$pkg" | fzf -i --multi --exact --no-sort --ansi --reverse --bind=right:half-page-down,left:half-page-up --margin=4%,1%,1%,1% --info=inline --no-unicode --preview '
                    if ( pacman -Qq "$( echo -e {} | sed "s/[\|\`\ -]*//" | cut -d " " -f 1 )" &>/dev/null )                                                 # check, if selected line contains a locally installed package.
                    then
                        echo -e "\e[1mInstalled package info: \e[0m"
                        pacman -Qi "$( echo -e {} | sed "s/[\|\`\ -]*//" | cut -d " " -f 1 )" --color always                                                 # display local package information in preview window of fzf
                    ...
                    ' "$( if (( $(tput cols) >= 125 )); then echo "--preview-window=right:55%:wrap"; else echo "--preview-window=bottom:55%:wrap"; fi )" --header="Repository Reverse Dependency Tree of \"$pkg\". ESC to quit." --prompt='Enter string to filter list > ' > /tmp/pacui-rt
...
function func_l
{
        ...
        pkg="$( sort -k1,1 -u /tmp/pacui-packages-local | fzf -i --exact --no-sort --select-1 --query="$argument_input" --cycle --reverse --bind=right:half-page-down,left:half-page-up --margin="4%,1%,1%,2%" --info=inline --no-unicode --preview '
            ...
            ' "$( if (( $(tput cols) >= 125 )); then echo "--preview-window=right:55%:wrap"; else echo "--preview-window=bottom:55%:wrap"; fi )" --header="ENTER to list files. ESC to quit." --prompt='Enter string to filter list > ' | awk '{print $1}' )"
        ...
        then
            ...
            then
                ...
                pacman -Ql "$pkg" --color always 2>/dev/null | grep -a -v "/$" | awk '{print $NF}' | fzf -i --multi --exact --no-sort --query="usr/bin/" --reverse --bind=right:half-page-down,left:half-page-up --margin=4%,1%,1%,1% --info=inline --header="List of files of local package \"$pkg\". Press ESC to quit." --prompt='Manipulate string to filter list > ' > /tmp/pacui-l
                ...
                then
                    ...
                    # in some cases, the local database has to be initialized first with "sudo pacman -Fyy"
                    sudo pacman -Fy
                fi
                ...
                pacman -Fl --machinereadable "$pkg" | grep -a -v "/$" | awk -F '\0' '{print $4}' | fzf -i --multi --exact --no-sort --query="usr/bin/" --reverse --bind=right:half-page-down,left:half-page-up --margin=4%,1%,1%,1% --info=inline --header="List of files of remote package \"$pkg\". Press ESC to quit." --prompt='Manipulate string to filter list > ' > /tmp/pacui-l
...
function func_s
{
        ...
        then
            ...
            local {pacui_search_temp_local,pacui_search_temp_repo2,pacui_search_temp_only_in_repo2,server}
            ...
            cat /tmp/pacui-search | fzf -i --multi --exact --no-sort --ansi --reverse --bind=right:half-page-down,left:half-page-up --margin=4%,1%,1%,1% --info=inline --header="Package file names and paths containing \"$file\". Press ESC to quit." --prompt='Enter string to filter list > ' > /tmp/pacui-s
...
function func_f
{
        ...
        local dbpath="$( awk -F '=' '/^DBPath/ {gsub(" ","",$2); print $2}' '/etc/pacman.conf' )"               # extract path of database file from pacman.conf
        ...
        then
            ...
            then
                sudo reflector --verbose --protocol https --age 1 --sort rate --save /etc/pacman.d/mirrorlist && sleep 10 && sudo pacman -Syy
            fi
            echo ""
        fi
...
function func_c
{
        ...
        [[ -f /etc/default/tlp ]]                       && echo -e "/etc/default/tlp                                                                                                                                                                                                                                                 Configure power management in file:" >> ${pacui_config}
        ...
        [[ -f /boot/loader/entries ]]                   && echo -e "\e[31m/boot/loader/entries/                                                                                                                                                                                                                                      Configure systemd-boot boot loader entries:\e[0m" >> ${pacui_config}
        ...
        file="$( echo -e "$( cat ${pacui_config} )" | fzf -i --exact --no-sort --select-1 --ansi --cycle --query="$argument_input" --reverse --bind=right:half-page-down,left:half-page-up --margin="4%,1%,1%,2%" --info=inline --no-unicode --preview '
            ...
            ' "$( if (( $(tput cols) >= 125 )); then echo "--preview-window=right:60%:wrap"; else echo "--preview-window=bottom:60%:wrap"; fi )" --header="ENTER to continue. ESC to quit." --prompt='Enter string to filter list > ' | awk '{print $1}' )"
        ...
        then
            ...
            then
                ...
                file="$( find "$file" -maxdepth 1 -xtype f | sort -u | fzf -i --exact --no-sort --select-1 --reverse --bind=right:half-page-down,left:half-page-up --margin="4%,1%,1%,2%" --info=inline --no-unicode --preview '
                    ...
                    ' "$( if (( $(tput cols) >= 125 )); then echo "--preview-window=right:55%:wrap"; else echo "--preview-window=bottom:55%:wrap"; fi )" --header="ENTER to edit file in text editor. ESC to quit." --prompt='Enter string to filter list > ' )"
...
function func_ls
{
        ...
        # $( comm -23 <(pacman -Qq | sort) <(pacman -Qqg base | sort) )  only shows packages, which are not part of "base" group.
        ...
        expac -H M -Q "%12m - \e[1m%n\e[0m %v" $( comm -23 <(pacman -Qq | sort) <(pacman -Qqg base | sort) ) | sort -n -r | fzf -i --multi --exact --no-sort --ansi --reverse --bind=right:half-page-down,left:half-page-up --margin=4%,1%,1%,1% --info=inline --no-unicode --preview '
            echo -e "\e[1mInstalled package info: \e[0m"
...
function func_la
{
        ...
        if ( wget --spider "https://aur.archlinux.org" &>/dev/null )                # the "wget --spider" command gets executed in any case in order to check its output.
        then
            AurSserverIsUp="true"
        else
        ...
        pacman -Qqm --color always | fzf -i --multi --exact --no-sort --ansi --reverse --bind=right:half-page-down,left:half-page-up --margin="4%,1%,1%,2%" --info=inline --no-unicode --preview '
            echo -e "\e[1mInstalled package info: \e[0m"
            ...
            ' "$( if (( $(tput cols) >= 125 )); then echo "--preview-window=right:60%:wrap"; else echo "--preview-window=bottom:60%:wrap"; fi )" --header="List of manually installed packages. ESC to quit." --prompt='Enter string to filter list > ' > /tmp/pacui-la
...
function func_d
{
        ...
        then
            ...
            pkg="$( sort -k1,1 -u /tmp/pacui-packages-local | fzf -i --multi --exact --no-sort --select-1 --bind=right:half-page-down,left:half-page-up --query="$argument_input" --cycle --reverse --margin="4%,1%,1%,2%" --info=inline --no-unicode --preview '
                if ( pacman -Qq {1} &>/dev/null )                                   # check, if selected line is a locally installed package
                then
                    echo -e "\e[1mInstalled package info: \e[0m"
                    pacman -Qi {1} --color always                                   # for local packages, local query is sufficient.
                ...
                ' "$( if (( $(tput cols) >= 125 )); then echo "--preview-window=right:55%:wrap"; else echo "--preview-window=bottom:55%:wrap"; fi )" --header="TAB to (un)select. ENTER to downgrade. ESC to quit." --prompt='Enter string to filter list > ' | awk '{print $1}' )"
            ...
            if [[ -n "$pkg" ]]
            then
                downgrade "$argument_flag"$pkg_downgrade                            # ATTENTION: (i do not know why but) using quotes (" symbols) around $pkg_downgrade variable breaks AUR helper and pacman
            fi
...
PacUI is an interactive package manager for your command line terminal. It provides an easy user interface and uses Pacman and Yay/Pikaur/Aurman/Pakku/Trizen/Pacaur as back ends.
...
PacUI uses Fuzzy Finder (fzf) to display selectable lists, which can be easily searched by starting to type. Advanced users can even use regular expressions to search in fzf. Navigate fzf's list the same way you navigate this help page.
...
If updates from system repositories fail, the user is offered the choice to update packages using \e[36m"sudo pacman -Syu --overwrite A-Z,a-z,0-9,-,.,_"\e[0m.
...
\e[36m"sudo pacman-mirrors -f 0 && sudo pacman -Syyu" \e[0m(for Manjaro)
This command generates a new mirrorlist of all available Manjaro repository mirrors/servers and sorts it by ping of up-to-date mirrors/servers. Additionally, the latest package database is downloaded from the chosen Manjaro repository mirror and the system is updated. If you want to speed up this command, it is recommended to only test your connection quality to Manjaro mirrors/servers near you. Example: You have noticed the pings to German and French mirrors are always best for you. Then, you can run: "sudo pacman-mirrors -f 0 -c Germany,France". In order to prevent partial updates, the whole system needs to be updated after the change to another repository mirror/server.
Here a short explanation of partial updates and why a complete system update is needed: Assume you switch to another (updated) repository mirror/server with "sudo pacman-mirrors -f 0". Afterwards, a sync of the local package database and the package database on the server is needed using "sudo pacman -Syy" (and updates are found). It is now possible you install a package using "sudo pacman -S", which also installs a new (and no longer compatible) version of "bash" as a dependency. This means that all other (and not updated) packages, which depend on "bash", are broken.

\e[36m"sudo reflector --verbose --protocol https --age 1 --sort rate --save /etc/pacman.d/mirrorlist && sudo pacman -Syyu" \e[0m(for Arch and other Arch-based distros)
This command generates a new mirrorslist of secure and fast repository mirrors/servers. Then it resyncs database to match the mirrors and updates the system.
...
\e[36m"comm -13 <(echo "$available_kernels") <(echo "$installed_kernels")"
...
\e[36m"yaourt -Sy <PACKAGE NAME>"
This option first updates your system and then downloads and installs <PACKAGE NAME> on your system. The list of packages shows packages from your system repository with their description while package groups or packages from the AUR are only shown with their name.
...
\e[36m"sudo reflector --verbose --protocol https --age 1 --sort rate --save /etc/pacman.d/mirrorlist && sudo pacman -Syy" \e[0m(For Arch and other Arch-based distributions)
...
\e[36m"yaourt -Syua --devel --needed"
...
This command updates both regular and AUR packages. Some AUR helpers need the "--devel" flag in order to update development versions (i.e. all git, svn, and cvs-packages) from the AUR as well.
...
function func_h
{
        ...
        echo
        echo -e "  1   pacui  u    - \e[1mU\e[0mpdate System"
        echo -e "  2   pacui  m    - \e[1mM\e[0maintain System"
        echo -e "  3   pacui  i    - \e[1mI\e[0mnstall Packages"
        echo -e "  4   pacui  r    - \e[1mR\e[0memove Packages and Deps"
        echo -e "  5   pacui  t    - Dependency \e[1mT\e[0mree"
        echo -e "  6   pacui  rt   - \e[1mR\e[0meverse Dependency \e[1mT\e[0mree"
        echo -e "  7   pacui  l    - \e[1mL\e[0mist Package Files"
        echo -e "  8   pacui  s    - \e[1mS\e[0mearch Package Files"
        echo
        echo -e "  9   pacui  b    - Roll \e[1mB\e[0mack System"
        echo -e " \e[31m 10  pacui  f    - \e[1mF\e[0;31mix Pacman Errors\e[0m"
        echo -e "  11  pacui  c    - Edit \e[1mC\e[0monfig Files"
        echo -e "  12  pacui  ls   - \e[1mL\e[0mist Packages by \e[1mS\e[0mize"
        echo
        [[ -n "$AUR_Helper" ]]      && echo -e "  13  pacui  ua   - Force \e[1mU\e[0mpdate \e[1mA\e[0mUR"
        echo -e "  14  pacui  la   - \e[1mL\e[0mist Installed from \e[1mA\e[0mUR"
        [[ -n "$AUR_Helper" ]]      && echo
        ...
        echo -e "  99  pacui  help - Full \e[1mHelp\e[0m page. 'q' key quits."
}
...
if [[ "$AUR_Helper" == "pacaur" ]] && [[ ! -f $HOME/.config/pacaur/config ]] && [[ ! -n $EDITOR ]] && [[ ! -f /usr/bin/vi ]]
then
...
do
    ...
    case "$key" in
        ...
        1|u|update|update-system)                                                   # the following commands will get executed if $key is "1" or "u" or "update" until a code line containing only ";;" is encountered
            function_call="u"
            shift                                                                   # shift to next argument (which is currently known as '$2'), and make it first argument ( referred to as '$1'). this command throws away the current argument, because we have done everything with it we wanted to do and it is no longer needed.
            ;;
        ...
        5|t|tree|dependency-tree )
            ...
            shift
            ;;
        6|rt|reversetree|reverse-dependency-tree )
            function_call="rt"
            shift
            ;;
        ...
        9|b|back|roll-back-system )
            ...
            shift
            ;;
        10|f|fix|fix-pacman-errors )
            function_call="f"
            shift
            ;;
        11|c|conf|config|edit-config-files )
            function_call="c"
            shift
            ;;
        12|ls|listsize|list-packages-by-size )
            function_call="ls"
            shift
            ;;
        13|ua|fua|forceupdateaur|updateaur|force-update-aur )
            function_call="ua"
            shift
            ;;
        14|la|listaur|list-installed-from-aur )
            function_call="la"
            shift
            ;;
    ...
    # print error message when more than 1 argument is recognized as a argument calling a pacui function
...
do
    ...
    echo -e " \e[1m+------------------------------------------------------------------+\e[0m"
    echo -e " \e[1m|\e[0m     1   \e[1mU\e[0mpdate System            2   \e[1mM\e[0maintain System             \e[1m|\e[0m"
    echo -e " \e[1m|\e[0m     3   \e[1mI\e[0mnstall Packages         4   \e[1mR\e[0memove Packages and Deps    \e[1m|\e[0m"
    echo -e " \e[1m|\e[0m------------------------------------------------------------------\e[1m|\e[0m"
    echo -e " \e[1m|\e[0m     5   Dependency \e[1mT\e[0mree          6   \e[1mR\e[0meverse Dependency \e[1mT\e[0mree     \e[1m|\e[0m"
    echo -e " \e[1m|\e[0m     7   \e[1mL\e[0mist Package Files       8   \e[1mS\e[0mearch Package Files        \e[1m|\e[0m"
    echo -e " \e[1m+------------------------------------------------------------------+\e[0m"
    echo -e "       9   Roll \e[1mB\e[0mack System       \e[31m 10   \e[1mF\e[0;31mix Pacman Errors\e[0m "
    echo -e "      11   Edit \e[1mC\e[0monfig Files       12   \e[1mL\e[0mist Packages by \e[1mS\e[0mize "
    echo -e " \e[1m+------------------------------------------------------------------+\e[0m"
    [[ -n "$AUR_Helper" ]] && echo -e " \e[1m|\e[0m    13   Force \e[1mU\e[0mpdate \e[1mA\e[0mUR        14   \e[1mL\e[0mist Installed from \e[1mA\e[0mUR     \e[1m|\e[0m"
    [[ -z "$AUR_Helper" ]] && echo -e " \e[1m|\e[0m                                 14   \e[1mL\e[0mist Installed from \e[1mA\e[0mUR     \e[1m|\e[0m"
    echo -e " \e[1m+------------------------------------------------------------------+\e[0m"
    echo
    echo -e "  Enter number or marked letter(s)      -   99   \e[1mH\e[0melp   -   0   \e[1mQ\e[0muit "
    ...
    case "$choice" in
        ...
        1|u|update|update-system )                                                  # the following commands will get executed if $choice is "1" or "u" or "update" until a code line containing only ";;" is encountered
